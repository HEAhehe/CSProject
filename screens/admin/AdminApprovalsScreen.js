...
// ✅ เพิ่ม addDoc สำหรับการส่งแจ้งเตือน
...
export default function AdminApprovalsScreen({ navigation }) {
  ...
  const getRequestTypeColor = (type) => type === 'store_update' ? '#8b5cf6' : '#3b82f6';

  const handleRequestPress = (request) => { setSelectedRequest(request); setModalVisible(true); };
  ...
  return (
    <View style={styles.container}>
      ...
      {loading ? (
        ...
        <FlatList data={filteredRequests} renderItem={renderRequest} keyExtractor={(item) => item.id} contentContainerStyle={styles.requestsList} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} />} />
      ) : (
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            <View style={styles.modalHeader}>
              ...
              <TouchableOpacity onPress={() => { setModalVisible(false); setRejectReason(''); }}><Ionicons name="close" size={24} color="#1f2937" /></TouchableOpacity>
            </View>
            {selectedRequest && (
              <View style={{flex: 1}}>
                <ScrollView style={styles.modalBody} showsVerticalScrollIndicator={false}>
                  <View style={styles.infoRow}><Text style={styles.label}>ประเภท:</Text><Text style={styles.value}>{selectedRequest.type === 'store_update' ? 'แก้ไขข้อมูลร้าน' : 'เปิดร้านใหม่'}</Text></View>
                  <View style={styles.infoRow}><Text style={styles.label}>ผู้ขอ:</Text><Text style={styles.value}>{selectedRequest.userName}</Text></View>
                  ...
                  {selectedRequest.details && Object.entries(selectedRequest.details).map(([key, value]) => (
                    <View key={key} style={styles.infoRow}><Text style={styles.label}>{key}:</Text><Text style={styles.value}>{String(value)}</Text></View>
                  ))}
                  ...
                  {selectedRequest.status === 'rejected' && selectedRequest.rejectReason && (
                    <View style={styles.rejectReasonBox}><Text style={styles.rejectReasonTitle}>เหตุผลที่ปฏิเสธ:</Text><Text style={styles.rejectReasonText}>{selectedRequest.rejectReason}</Text></View>
                  )}
                  <View style={{height: 40}} />
                </ScrollView>
                {selectedRequest.status === 'pending' && (
                  <View style={styles.modalActions}>
                    <TouchableOpacity style={styles.rejectButton} onPress={handleReject}><Ionicons name="close-circle" size={20} color="#fff" /><Text style={styles.buttonText}>ปฏิเสธ</Text></TouchableOpacity>
                    <TouchableOpacity style={styles.approveButton} onPress={handleApprove}><Ionicons name="checkmark-circle" size={20} color="#fff" /><Text style={styles.buttonText}>อนุมัติ</Text></TouchableOpacity>
                  </View>
                )}
              </View>
            )}
