    ...
    if (user) {
      const unsubscribe = onSnapshot(doc(db, 'users', user.uid), (doc) => {
        if (doc.exists()) {
          ...
          if (!pickedImageBase64) setProfileImage(data.profileImage || null);
        }
      });
      return () => unsubscribe();
    }
    ...
    <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <TouchableOpacity style={styles.saveButton} onPress={handleSave} disabled={loading}>
          {loading ? <ActivityIndicator color="#10b981" size="small" /> : <Text style={styles.saveText}>บันทึก</Text>}
        </TouchableOpacity>
      </View>
        ...
        <View style={styles.formSection}>
          <View style={styles.inputGroup}>
            ...
            <View style={styles.inputContainer}>
              ...
              <TextInput style={styles.input} placeholder="ชื่อผู้ใช้" value={username} onChangeText={setUsername} autoCapitalize="none" placeholderTextColor="#9ca3af" />
            </View>
          </View>
          ...
          <View style={styles.inputGroup}>
            ...
            <View style={[styles.inputContainer, styles.inputDisabled]}>
              ...
              <TextInput style={[styles.input, styles.textDisabled]} value={userData?.email || ''} editable={false} placeholderTextColor="#9ca3af" />
          ...
          <View style={styles.inputGroup}>
            ...
            <View style={styles.inputContainer}>
              ...
              <TextInput style={styles.input} placeholder="เบอร์โทรศัพท์" value={phoneNumber} onChangeText={setPhoneNumber} keyboardType="phone-pad" placeholderTextColor="#9ca3af" />
            </View>
          </View>

          <View style={styles.infoSection}>
            <View style={styles.infoRow}><Text style={styles.infoLabel}>สมาชิกตั้งแต่</Text><Text style={styles.infoValue}>{userData?.createdAt ? new Date(userData.createdAt).toLocaleDateString('th-TH') : '-'}</Text></View>
            <View style={styles.infoDivider} />
            <View style={styles.infoRow}><Text style={styles.infoLabel}>อัปเดตล่าสุด</Text><Text style={styles.infoValue}>{userData?.updatedAt ? new Date(userData.updatedAt).toLocaleDateString('th-TH') : '-'}</Text></View>
          </View>
        </View>
...
const styles = StyleSheet.create({
  ...
  saveText: { fontSize: 16, fontWeight: '600', color: '#10b981' },
  content: { flex: 1 },
  imageSection: { alignItems: 'center', paddingVertical: 30, backgroundColor: '#fff', marginBottom: 15 },
  ...
  changePhotoText: { fontSize: 14, color: '#10b981', fontWeight: '500' },
  formSection: { backgroundColor: '#fff', paddingHorizontal: 20, paddingVertical: 20, marginBottom: 15 },
  inputGroup: { marginBottom: 20 },
  label: { fontSize: 14, fontWeight: '600', color: '#374151', marginBottom: 8 },
  inputContainer: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#f9fafb', borderRadius: 12, paddingHorizontal: 15, borderWidth: 1, borderColor: '#e5e7eb' },
  inputDisabled: { backgroundColor: '#f3f4f6' },
  icon: { marginRight: 10 },
  input: { flex: 1, paddingVertical: 14, fontSize: 15, color: '#1f2937' },
  textDisabled: { color: '#9ca3af' },
  helperText: { fontSize: 12, color: '#9ca3af', marginTop: 5, marginLeft: 5 },
  infoSection: { backgroundColor: '#f9fafb', borderRadius: 12, padding: 15, marginTop: 10 },
