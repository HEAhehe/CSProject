...
export default function CreateListingScreen({ navigation, route }) {
  ...
  // ✅ ฟังก์ชันแจ้งเตือนที่รับ ID สินค้าและราคาไปด้วย
        ...
        for (const userDoc of usersSnapshot.docs) {
          ...
          if (favSnap.exists()) {
            ...
            batch.set(notifRef, {
              ...
              // 👇 3 บรรทัดนี้คือหัวใจสำคัญที่ของเดิมในเครื่องคุณหายไปครับ 👇
              foodId: foodId,
              price: price,
    ...
    try {
      ...
      if (isEditMode) {
        ...
        Alert.alert('สำเร็จ', 'อัปเดตสินค้าเรียบร้อยแล้ว', [{ text: 'ตกลง', onPress: () => navigation.goBack() }]);
              ...
              // 1. สร้างสินค้าและเก็บ ID
              const docRef = await addDoc(collection(db, 'food_items'), payload);

              // 2. เรียกแจ้งเตือน (ต้องมีตัวแปรครบ 7 ตัว)
              await notifyFollowers(
                ...
                imageUri || null,
                docRef.id,             // <--- ส่ง ID
                Number(discountPrice), // <--- ส่งราคา
                Number(originalPrice)  // <--- ส่งราคาเต็ม
              );
    ...
    <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={styles.container}>
      ...
      <ScrollView style={styles.content} contentContainerStyle={{ paddingBottom: 40 }} showsVerticalScrollIndicator={false}>
        <TouchableOpacity style={styles.imagePicker} onPress={pickImage} activeOpacity={0.8}>
          {imageUri ? (
            <><Image source={{ uri: imageUri }} style={styles.pickedImage} /><View style={styles.imageEditBadge}><Ionicons name="camera" size={16} color="#fff" /><Text style={styles.imageEditText}>เปลี่ยนรูป</Text></View></>
          ) : (
            <View style={styles.imagePlaceholder}><View style={styles.imagePlaceholderIcon}><Ionicons name="camera-outline" size={36} color="#10b981" /></View><Text style={styles.imagePlaceholderTitle}>เพิ่มรูปสินค้า</Text><Text style={styles.imagePlaceholderSub}>แตะเพื่ออัปโหลดรูปภาพ</Text></View>
          )}
        </TouchableOpacity>

        <View style={styles.formSection}>
          <View style={styles.fieldGroup}><Text style={styles.fieldLabel}>ชื่อสินค้า <Text style={styles.required}>*</Text></Text><TextInput style={styles.input} placeholder="เช่น ข้าวกล่อง..." placeholderTextColor="#9ca3af" value={name} onChangeText={setName} /></View>
          <View style={styles.fieldGroup}><Text style={styles.fieldLabel}>หมวดหมู่</Text><TouchableOpacity style={styles.selectInput} onPress={() => setShowCategoryModal(true)}><View style={styles.selectLeft}><Ionicons name={CATEGORY_OPTIONS.find(c => c.label === category)?.icon || 'cube-outline'} size={18} color="#10b981" /><Text style={styles.selectText}>{category}</Text></View><Ionicons name="chevron-down" size={18} color="#9ca3af" /></TouchableOpacity></View>

          <View style={styles.fieldRow}>
            <View style={[styles.fieldGroup, { flex: 1 }]}><Text style={styles.fieldLabel}>ราคาปกติ (฿)</Text><TextInput style={styles.input} placeholder="0" keyboardType="numeric" value={originalPrice} onChangeText={setOriginalPrice} /></View>
            <View style={[styles.fieldGroup, { flex: 1 }]}><Text style={styles.fieldLabel}>ราคาขาย (฿) <Text style={styles.required}>*</Text></Text><TextInput style={[styles.input, styles.inputHighlight]} placeholder="0" keyboardType="numeric" value={discountPrice} onChangeText={setDiscountPrice} /></View>
          </View>

          {discountPercent > 0 && <View style={styles.discountPreview}><Ionicons name="pricetag" size={14} color="#10b981" /><Text style={styles.discountPreviewText}>ลด {discountPercent}% จากราคาปกติ</Text></View>}

          <View style={styles.fieldRow}>
            <View style={[styles.fieldGroup, { flex: 1 }]}><Text style={styles.fieldLabel}>จำนวน <Text style={styles.required}>*</Text></Text><TextInput style={styles.input} placeholder="0" keyboardType="numeric" value={quantity} onChangeText={setQuantity} /></View>
            <View style={[styles.fieldGroup, { flex: 1 }]}><Text style={styles.fieldLabel}>หน่วย</Text><TouchableOpacity style={styles.selectInput} onPress={() => setShowUnitModal(true)}><Text style={styles.selectText}>{unit}</Text><Ionicons name="chevron-down" size={18} color="#9ca3af" /></TouchableOpacity></View>
          </View>

          <View style={styles.fieldGroup}><Text style={styles.fieldLabel}>วันหมดอายุ / วันสุดท้ายที่ควรรับ</Text><TextInput style={styles.input} placeholder="เช่น 25/12/2567" value={expiryDate} onChangeText={setExpiryDate} /></View>
          <View style={styles.fieldGroup}><Text style={styles.fieldLabel}>รายละเอียดสินค้า</Text><TextInput style={[styles.input, styles.inputMultiline]} placeholder="บอกเล่าเกี่ยวกับสินค้า..." multiline numberOfLines={4} textAlignVertical="top" value={description} onChangeText={setDescription} /></View>
          <View style={styles.fieldGroup}><Text style={styles.fieldLabel}>หมายเหตุการรับสินค้า</Text><TextInput style={styles.input} placeholder="เช่น รับที่หน้าร้าน" value={pickupNote} onChangeText={setPickupNote} /></View>
        </View>
        ...
        {name.trim() && discountPrice ? (
          <View style={styles.summaryCard}>
            ...
            <View style={styles.summaryRow}>
              <View style={styles.summaryImageWrap}>{imageUri ? <Image source={{ uri: imageUri }} style={styles.summaryImage} /> : <View style={[styles.summaryImage, { backgroundColor: '#d1fae5', justifyContent: 'center', alignItems: 'center' }]}><Ionicons name="fast-food-outline" size={24} color="#10b981" /></View>}</View>
              <View style={{ flex: 1 }}>
                ...
                <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8, marginTop: 4 }}>
                  ...
                  {discountPercent > 0 && <View style={styles.summaryBadge}><Text style={styles.summaryBadgeText}>-{discountPercent}%</Text></View>}
                </View>
      ...
      <View style={styles.footer}>
        <TouchableOpacity style={[styles.submitBtn, loading && { opacity: 0.7 }]} onPress={handleSubmit} disabled={loading} activeOpacity={0.85}>
          {loading ? <ActivityIndicator color="#fff" /> : <><Ionicons name={isEditMode ? 'checkmark-circle-outline' : 'cloud-upload-outline'} size={20} color="#fff" /><Text style={styles.submitBtnText}>{isEditMode ? 'บันทึกการแก้ไข' : 'โพสต์สินค้า'}</Text></>}
        </TouchableOpacity>
      </View>

      <Modal visible={showUnitModal} transparent animationType="fade" onRequestClose={() => setShowUnitModal(false)}><TouchableOpacity style={styles.modalOverlay} activeOpacity={1} onPress={() => setShowUnitModal(false)}><View style={styles.modalBox}><Text style={styles.modalTitle}>เลือกหน่วย</Text>{UNIT_OPTIONS.map(u => (<TouchableOpacity key={u} style={[styles.modalOption, u === unit && styles.modalOptionActive]} onPress={() => { setUnit(u); setShowUnitModal(false); }}><Text style={[styles.modalOptionText, u === unit && styles.modalOptionTextActive]}>{u}</Text>{u === unit && <Ionicons name="checkmark" size={18} color="#10b981" />}</TouchableOpacity>))}</View></TouchableOpacity></Modal>
      <Modal visible={showCategoryModal} transparent animationType="fade" onRequestClose={() => setShowCategoryModal(false)}><TouchableOpacity style={styles.modalOverlay} activeOpacity={1} onPress={() => setShowCategoryModal(false)}><View style={styles.modalBox}><Text style={styles.modalTitle}>เลือกหมวดหมู่</Text>{CATEGORY_OPTIONS.map(c => (<TouchableOpacity key={c.label} style={[styles.modalOption, c.label === category && styles.modalOptionActive]} onPress={() => { setCategory(c.label); setShowCategoryModal(false); }}><View style={{ flexDirection: 'row', alignItems: 'center', gap: 12 }}><Ionicons name={c.icon} size={18} color={c.label === category ? '#10b981' : '#6b7280'} /><Text style={[styles.modalOptionText, c.label === category && styles.modalOptionTextActive]}>{c.label}</Text></View>{c.label === category && <Ionicons name="checkmark" size={18} color="#10b981" />}</TouchableOpacity>))}</View></TouchableOpacity></Modal>
